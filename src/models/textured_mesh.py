...
def build_cotan_laplacian_torch(
    ...
    vals = np.concatenate((wab, wab, -wab, -wab, wbc, wbc, -wbc, -wbc, wca, wca, -wca, -wca),
                          axis=0)
    L = sparse.coo_matrix((vals, (rows, cols)),
                          shape=(points.shape[1], points.shape[1]),
                          dtype=float).tocsc()
    return L
...
class TexturedMeshModel(nn.Module):
    def __init__(
        ...
        if self.opt.reference_texture:
            ...
            ).permute(2, 0, 1).cuda().unsqueeze(0) / 255.0
            change_mask = ((base_texture.to(self.device) - self.texture_img).abs().sum(axis=1) >
                           0.1).float()
            with torch.no_grad():
    ...
    def spectral_augmentations(self, vertices: torch.Tensor) -> torch.Tensor:
        ...
        basis_functions /= basis_functions.max(dim=-1)[0][:, None] - basis_functions.min(dim=-1
                                                                                        )[0][:,
                                                                                             None]
        ...
        signs = ((torch.rand(len(chosen_basis_function), device=self.device) > 0.5).float() -
                 0.5) * 2
    ...
    def axis_augmentations(
        self, vertices: torch.Tensor, stretch_factor: float = 1.6, squish_factor: float = 0.7
    ):
    ...
    def init_paint(self, num_backgrounds=1):
        ...
        modulated_init_background_bases_latent = init_background_bases[:, None, None, :
                                                                      ] * 0.8 + 0.2 * torch.randn(
                                                                          num_backgrounds,
                                                                          self.env_sphere.faces.
                                                                          shape[0],
                                                                          3,
                                                                          self.num_features,
                                                                          dtype=torch.float32
        ...
        if self.initial_texture_path is not None:
            ...
            texture = torch.ones(1, 3, self.texture_resolution, self.texture_resolution).cuda(
    ...
    def change_default_to_median(self):
        diff = (
            self.texture_img - torch.tensor(self.default_color).view(1, 3, 1, 1).to(self.device)
        ).abs().sum(axis=1)
        default_mask = (diff < 0.1).float().unsqueeze(0)
        median_color = self.texture_img[0, :].reshape(3,
                                                      -1)[:,
                                                          default_mask.flatten() == 0].mean(axis=1)
        with torch.no_grad():
            self.texture_img.reshape(3,
                                     -1)[:,
                                         default_mask.flatten() == 1] = median_color.reshape(-1, 1)
    ...
    def init_texture_map(self):
        ...
        if cache_path is None:
            ...
            vt_cache, ft_cache = cache_path / 'vt.pt', cache_path / 'ft.pt'
        ...
        if self.mesh.vt is not None and self.mesh.ft is not None \
            ...
            run_xatlas = False
            # run_xatlas = not (
            #     vt.shape[0] == self.mesh.vertices.shape[0] and
            #     ft.shape[0] == self.mesh.faces.shape[0]
            # )
    ...
    def render(
        ...
        background_sphere_colors = self.background_sphere_colors[torch.randint(
            0, self.background_sphere_colors.shape[0], (1, )
        )]
        if use_meta_texture:
        ...
        if use_median:
            diff = (
                texture_img - torch.tensor(self.default_color).view(1, 3, 1, 1).to(self.device)
            ).abs().sum(axis=1)
            default_mask = (diff < 0.1).float().unsqueeze(0)
            median_color = texture_img[0, :].reshape(3,
                                                     -1)[:,
                                                         default_mask.flatten() == 0].mean(axis=1)
            texture_img = texture_img.clone()
            with torch.no_grad():
                texture_img.reshape(3,
                                    -1)[:,
                                        default_mask.flatten() == 1] = median_color.reshape(-1, 1)
        ...
        return {
            'image': pred_map, 'mask': mask, 'background': pred_back, 'foreground': pred_features,
            'depth': depth, 'normals': normals, 'render_cache': render_cache, 'texture_map':
            texture_img
        }
