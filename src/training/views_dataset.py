...
def rand_modal_poses(
    ...
    if overhead_flag:
        ...
        phis = rand_pertubations + torch.from_numpy(phi_mods[np.random.randint(0, 4, size)]
                                                   ).to(device)
        thetas = torch.rand(size,
                            device=device) * (theta_range[1] - theta_range[0]) + theta_range[0]
...
class MultiviewDataset:
    def __init__(self, cfg: RenderConfig, device):
        ...
        # Alternate lists
        alternate_lists = lambda l: [l[
            0]] + [i for j in zip(l[1:size // 2], l[-1:size // 2:-1]) for i in j] + [l[size // 2]]
