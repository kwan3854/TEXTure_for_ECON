...
from src.training.views_dataset import MultiviewDataset, ViewsDataset
...
class TEXTure:
    def __init__(self, cfg: TrainConfig):
        ...
        self.back_im = torch.Tensor(
            np.array(Image.open(self.cfg.guide.background_img).convert('RGB'))
        ).to(self.device).permute(2, 0, 1) / 255.0

        logger.info(f'Successfully initialized {self.cfg.log.exp_name}')

    def init_mesh_model(self) -> nn.Module:
        cache_path = Path('cache') / Path("/".join(self.cfg.guide.shape_path.split("/")[:-1])).stem
        ...
        if not self.cfg.guide.append_direction:
            ...
            for d in self.view_dirs:
                ...
                negative_words = [
                    ...
                    'duplicate', 'extra fingers', 'extra limbs', 'fused fingers', 'long neck',
                    'low quality', 'lowres', 'malformed limbs', 'missing arms', 'mutated hands',
                    'mutilated', 'out of frame', 'poorly drawn face', 'poorly drawn hands',
                    'too many fingers', 'ugly', 'worst quality'
                ]
                negative_prompt = ", ".join(negative_words)
                logger.info(negative_prompt)
                text_z.append(
                    self.diffusion.get_text_embeds([text], negative_prompt=negative_prompt)
                )
    ...
    def init_dataloaders(self) -> Dict[str, DataLoader]:
        ...
        dataloaders = {
            'train': init_train_dataloader, 'val': val_loader, 'val_large': val_large_loader
        }
        return dataloaders
    ...
    def paint(self):
        ...
        pbar = tqdm(
            ...
            bar_format=
            '{desc}: {percentage:3.0f}% painting step {n_fmt}/{total_fmt} [{elapsed}<{remaining}, {rate_fmt}]'
        )
        ...
        for i, data in enumerate(dataloader):
            ...
            if save_as_video:
                ...
                Image.fromarray(pred).save(
                    save_path / f"step_{self.paint_step:05d}_{i:04d}_rgb.png"
                )
                Image.fromarray((cm.seismic(normals[0, 0].cpu().numpy())[:, :, :3] * 255).astype(
                    np.uint8
                )).save(save_path / f'{self.paint_step:04d}_{i:04d}_normals_cache.png')
    ...
    def paint_viewpoint(self, data: Dict[str, Any]):
        ...
        outputs = self.mesh_model.render(
            background=background, render_cache=render_cache, use_median=self.paint_step > 1
        )
        ...
        crop_info = utils.get_nonzero_region(outputs['mask'][0, 0])
        cropped_rgb_render = utils.crop_or_pad_image(rgb_render, crop_info)
        cropped_depth_render = utils.crop_or_pad_image(depth_render, crop_info)
        cropped_update_mask = utils.crop_or_pad_image(update_mask, crop_info)
        try:
        ...
        if self.paint_step > 1:
            checker_mask = self.generate_checkerboard(
                utils.crop_or_pad_image(update_mask, crop_info), 
                utils.crop_or_pad_image(refine_mask, crop_info), 
                utils.crop_or_pad_image(generate_mask, crop_info)
            )
        ...
        cropped_rgb_output = F.interpolate(
            cropped_rgb_output, (cropped_rgb_render.shape[2], cropped_rgb_render.shape[3]),
            mode='bilinear',
            align_corners=False
        )
        ...
        rgb_output[:, :, crop_info[0]:crop_info[2], crop_info[1]:crop_info[3]] = cropped_rgb_output
        # rgb_output = utils.crop_or_pad_image(cropped_rgb_output, crop_info)
    ...
    def eval_render(self, data):
        ...
        diff = (
            rgb_render.detach() -
            torch.tensor(self.mesh_model.default_color).view(1, 3, 1, 1).to(self.device)
        ).abs().sum(axis=1)
        uncolored_mask = (diff < 0.1).float().unsqueeze(0)
        rgb_render = rgb_render * (1 - uncolored_mask) + utils.color_with_shade(
        ...
        texture_rgb = outputs_with_median['texture_map'].permute(0, 2, 3,
                                                                 1).contiguous().clamp(0,
                                                                                       1).detach()
    ...
    def calculate_trimap(
        self, rgb_render_raw: torch.Tensor, depth_render: torch.Tensor, z_normals: torch.Tensor,
        z_normals_cache: torch.Tensor, edited_mask: torch.Tensor, mask: torch.Tensor
    ):
        diff = (
        ...
        generate_mask = torch.from_numpy(
            cv2.dilate(
                exact_generate_mask[0, 0].detach().cpu().numpy(), np.ones((19, 19), np.uint8)
            )
        ...
        if self.cfg.guide.initial_texture is None:
            ...
            refine_mask = torch.from_numpy(
                cv2.dilate(refine_mask[0, 0].detach().cpu().numpy(), np.ones((1, 1), np.uint8))
        ...
        if self.cfg.log.log_images:
            ...
            trimap_vis = trimap_vis * (1 - exact_generate_mask) + utils.color_with_shade(
                [255 / 255.0, 22 / 255.0, 67 / 255.0], z_normals=z_normals, light_coef=0.7
            ) * exact_generate_mask

            shaded_rgb_vis = rgb_render_raw.detach()
            shaded_rgb_vis = shaded_rgb_vis * (1 - exact_generate_mask) + utils.color_with_shade(
            ...
            if self.paint_step > 1 or self.cfg.guide.initial_texture is not None:
                ...
                only_old_mask_for_vis = torch.bitwise_and(
                    refine_mask == 1, exact_generate_mask == 0
                ).float().detach()
    ...
    def generate_checkerboard(
        ...
        only_old_mask = F.interpolate(
            torch.bitwise_and(improve_z_mask_inner == 1, update_mask_base_inner == 0).float(),
            (512, 512)
        )
    ...
    def project_back(
        self, render_cache: Dict[str, Any], background: Any, rgb_output: torch.Tensor,
        ...
        blurred_render_update_mask = torch.from_numpy(
            cv2.dilate(
                render_update_mask[0, 0].detach().cpu().numpy(), np.ones((25, 25), np.uint8)
            )
        ...
        optimizer = torch.optim.Adam(
            self.mesh_model.get_params(), lr=self.cfg.optim.lr, betas=(0.9, 0.99), eps=1e-15
        )
        for _ in tqdm(range(200), desc='fitting mesh colors'):
            ...
            loss = ((masked_pred - masked_target.detach()).pow(2) *
                    masked_mask).mean() + ((masked_pred - masked_pred.detach()).pow(2) *
                                           (1 - masked_mask)).mean()
            ...
            masked_current_z_normals = current_z_normals.reshape(1, current_z_normals.shape[1],
                                                                 -1)[:, :,
                                                                     current_z_mask == 1][:, :1]
            masked_last_z_normals = z_normals_cache.reshape(1, z_normals_cache.shape[1],
                                                            -1)[:, :, current_z_mask == 1][:, :1]
    ...
    def log_diffusion_steps(self, intermediate_vis: List[Image.Image]):
        if len(intermediate_vis) > 0:
            ...
            for k, intermedia_res in enumerate(intermediate_vis):
                intermedia_res.save(step_folder / f'{k:02d}_diffusion_step.png')
