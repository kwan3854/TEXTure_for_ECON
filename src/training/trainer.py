...
class TEXTure:
    ...
    def init_mesh_model(self) -> nn.Module:
        cache_path = (
            Path("cache")
            / Path("/".join(self.cfg.guide.shape_path.split("/")[:-1])).stem
        )
    ...
    def paint_viewpoint(self, data: Dict[str, Any]):
        ...
        if self.cfg.guide.reference_texture is not None and update_ratio < 0.00001:
            logger.info(
        ...
        rgb_output[
            :, :, crop_info[0] : crop_info[2], crop_info[1] : crop_info[3]
        ] = cropped_rgb_output
